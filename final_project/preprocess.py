...
DATASET_PATH_TESTING = \
    "C:\\Users\\thegr\\python\\school\\cv\\ecen-644-notebook\\datasets\\unzipped\\endovis\\Segmentation_Robotic_Training\\Testing"
...
def load_endovis_videos(dataset: DatasetType):
    ...
    input_frames_raw_pkl_path = \
        os.path.join(os.path.dirname(__file__), f"pickled\\input_frames_raw_{dataset.name.lower()}.pkl")
    output_frames_raw_pkl_path = \
        os.path.join(os.path.dirname(__file__), f"pickled\\output_frames_raw_{dataset.name.lower()}.pkl")
